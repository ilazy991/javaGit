class ArraySort {
	...
	public void bubbleSort() {
		 ...
		 * 内部循环第一次运行时,使用一个旗标来表示有无需要交换的可能,可以把最优时间复杂度降低到O(n) 平均时间复杂度 ---- O(n^2) 需辅助空间
		 * ------ O(1) 稳定性 ------------ 稳定
		 */
	...
	public void bubbleSort_Perfection1() {	/*
		* 冒泡算法的改进1 假如标志性变量，由于标志某一趟排序过程中是否有数据交换 如果进行某一趟排序时并没有进行数据 换，则说明数据已经按照要求排序好
		* 可立即结束排序
		*/
	...
	public void bubbleSort_Perfection2() {
		 ...
		 * ------------ 稳定 鸡尾酒排序 也叫定向冒泡排序，是冒泡排序的一种改进。 此算法与冒泡排序的不同处在于 低到高然后从高到低，
		 * 而冒泡排序则仅从低到高去比较序列里的每个元素。 他可以得到比冒泡排序稍 好一点的效能。
		 */
	...
	public void selectionSort() {
		 ...
		 * ------ O(1) 稳定性 ------------ 不稳定 首先在 排序序列中找到最小（大）元素，存放到排序序列的起始位置；
	...
	public void insertionSort_Perfection1() {
		/*
		 * 插入排序的改进：二分插入排序 最差时间复杂度 ---- O(n^2) 最优时间复杂度 ---- O(nlogn) 平均时间复杂度 ---- O(n^2)
		 * 所需辅助空间 ------ O(1) 稳定性 ------------ 稳定
		 * n较大时，二分插入排序的比较次数比直接插入排序的最差情况好得多，但比直接插入排序的最好情况要差，所当以元素初始序列已经接近升序时，直接插入排序比二分
		 * 插入排 序比较次数少。二分插入排序元素移动次数与直接插入排序相同，依赖于元素初始序列。
		 */
